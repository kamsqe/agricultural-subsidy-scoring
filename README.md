...
> **Мы уничтожили свою первую модель.**
>
> XGBoost доказал: главный предиктор одобрения — не качество фермы, а почтовый индекс.
> Реальные данные о бизнесе (урожайность, выживаемость, налоги) появятся только в **Едином Аграрном Реестре — 2027 г.**
> До этого любая команда, обучившая нейросеть на этом датасете — автоматизировала дискриминацию.
...
| # | Модуль | Для кого | Данные |
|---|--------|----------|--------|
| 1 | **Rules Engine (Impact Score)** | Комиссия | ✅ 36,651 реальных заявок |
| 2 | **ML Аудитор (XGBoost + SHAP)** | Комиссия | ✅ Реальный SHAP |
| 3 | **Anomaly Radar (Isolation Forest)** | Комиссия | ✅ 304 реальных аномалий |
| 4 | **Pre-Check** | Специалист УСХ | ✅ Та же формула |
| 5 | **Proactive Finder** | Специалист УСХ | ⚠️ Синт. профили + реальный движок |
| 6 | **Generative AI (Gemini)** | Комиссия / МСХ | ✅ Live API |
| 7 | **FIFO vs Merit Simulator** | МСХ | ✅ Реальная симуляция |
...
| **Агронормы КЗ** | Нормы падежа (по возрастным группам), инфраструктуры, критерии субсидий. Из [Приказа МСХ №3-3/1061](https://adilet.zan.kz/rus/docs/V1500012488) | `agroscore/public/data/agro_norms.json` |
| **Пастбищные нормы** | 253 записи нагрузки на пастбища по 17 областям × зоны × типы. Из [Приказа МСХ №3-1/52](https://adilet.zan.kz/rus/docs/V1500011064) | `agroscore/public/data/pasture_norms_official.json` |
...
| FIFO-парадоксы | 4,238 случаев: фермер подал первым — отказ. Поздний — одобрен |
...
### Модуль 4: Pre-Check (для специалиста УСХ)

Инструмент **специалиста районного УСХ** для консультации фермеров до подачи в ГИСС. Специалист вводит параметры заявки — система показывает предварительный балл, компоненты, контрафактуальный анализ и чеклист по причинам отказа. **Данные: реальные агрегаты.**

Потенциальный эффект: снижение retry rate с 30% до ~10%.

**Реализация:** `agroscore/src/components/PreCheck.tsx`

### Модуль 5: Generative AI (Gemini)

Два режима:
1. **AI-Ассистент** — ответы на вопросы Комиссии по конкретным заявкам (контекст = scoring_summary.json)
2. **Транслятор Закон → Код** — LLM транслирует текст нового Приказа МСХ в JSON-конфигурацию для Rules Engine

**Данные: реальные агрегаты + Live Gemini API.**

**Реализация:** `agroscore/src/components/AiAssistant.tsx` + `PdfToPython.tsx`
...
│   │   ├── PreCheck.tsx          # Консультация специалиста УСХ (та же формула)
...
| FIFO-парадоксов | 4,238 | Заявка раньше → отказ, позже → одобрено |
...
- **Pre-Check** — та же формула Impact Score, специалист УСХ вводит параметры для консультации фермера
